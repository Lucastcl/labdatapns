...
buscar_codigo <- function(termo, ano = NULL) {
  ...
  # Só executa a busca por termo se ele NÃO estiver vazio
  if (nchar(termo_limpo) > 0) {
    if (grepl("^[A-Z][0-9]{5}$", termo_limpo, ignore.case = TRUE)) {
      # Busca por código
      dados_filtrados <- dados_filtrados[stringr::str_to_upper(dados_filtrados$codigo) == stringr::str_to_upper(termo_limpo), ]
    } else {
  ...
  if (nrow(dados_filtrados) == 0) {
    # Não retorna mensagem se a busca inicial era vazia e não encontrou nada
    if (nchar(termo_limpo) > 0) {
  ...
  return(dados_filtrados[order(dados_filtrados$Ano, dados_filtrados$codigo), c("codigo", "pergunta", "Ano")])
}
