#' Calcula totais por UF da variável de interesse
#'
#' @param variaveis Vetor com nome(s) da(s) variável(is) de interesse.
#'
#' @return Data frame com totais, erro padrão, intervalo de confiança e coeficiente de variação por categoria e UF.
#' @export
...
#' @examples
#' tabela_totais_uf(variaveis = c("P040"))
tabela_totais_uf <- function(variaveis = c()) {
  ...
  total_var1_uf <- svytotal(
    reformulate(paste0("~interaction(V0001, ", vars[1], ")")),
    design = dados_pns_design,
    na.rm = TRUE
  )

  intervalo_total_var1_uf <- confint(total_var1_uf)
  cv_total_var1_uf <- cv(object = total_var1_uf)

  # Convertendo para data.frame
  total_var1_uf <- as.data.frame(total_var1_uf)
  intervalo_total_var1_uf <- as.data.frame(intervalo_total_var1_uf)
  cv_total_var1_uf <- as.data.frame(cv_total_var1_uf)

  # Função auxiliar para limpar rownames
  limpar_rownames_uf <- function(nome) {
    nome <- as.data.frame(nome)
    nomes_atuais <- rownames(nome)

    if (is.null(nomes_atuais)) {
      stop("O objeto fornecido não tem rownames.")
    }
    ...
    if (!any(partes_validas)) {
      stop("Nenhum nome de linha pôde ser dividido em duas partes por '.'")
    }
    ...
    nome[nome$categoria != "Ignorado", , drop = FALSE]
  }
  ...
  # Função para montar tabela final
  ...
  tabela_total_var1_uf <- criar_tabela_resumo(
    total = total_var1_uf,
    intervalo = intervalo_total_var1_uf,
    cv = cv_total_var1_uf
  )
